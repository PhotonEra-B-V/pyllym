---
name: pyllm-structured-prompting
description: >-
  Build structured prompting with the async-first pyllm library (Python 3.14):
  ...
  reviewing code that calls pyllm (pyllm.create_chat / embed / paint /
  animate), when
  designing a JSON-schema/Pydantic response contract, when a model id isn't
  found, or when wiring up a new AI provider.
---

# pyllm: structured prompting & new models

`pyllm` is one async-first Python framework for every major AI provider. This
...
chat = pyllm.create_chat(model="gpt-5.4").with_tools(Weather)
...
- Explicit params instead of signature inference — set a raw JSON-schema class
  ...
  ```
- Return `pyllm.SearchResults(title=..., url=..., text=...)` from a tool to give
...
```python
chat = (
    pyllm.create_chat(model="claude-sonnet-4-6")
...
```python
emb   = await pyllm.embed("hello")                                  # Embedding(vectors=...)
img   = await pyllm.paint("a red panda", provider="fal", model="fal-ai/flux/dev")
vid   = await pyllm.animate("a timelapse sunrise", model="fal-ai/ltx-video-13b-distilled")
audio = await pyllm.speak("hello", model="gpt-4o-mini-tts")
text  = await pyllm.transcribe("meeting.wav")
flags = await pyllm.moderate("some text")
```

## Including new models

The bundled registry (`models.json`) powers `pyllm.models.find(...)`. Three ways
to use a model that isn't in it yet:

1. **assume-exists** — pass the id + provider and skip the registry:
   ```python
   await pyllm.create_chat(model="deepseek-r2", provider="deepseek",
...
2. **refresh the registry** from providers + models.dev, then persist:
   ```python
   await pyllm.models.refresh()
   pyllm.models.save_to_json()
   ```
3. Inspect what's available: `pyllm.models.find("gpt-5.4")`,
   `pyllm.models.chat_models()`, `[m.id for m in pyllm.models.all() if m.provider == "gemini"]`.
...
Then register it in `src/pyllm/__init__.py` (add `("acme", "Acme")` to the
