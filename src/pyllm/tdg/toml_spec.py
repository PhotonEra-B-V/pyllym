...
A TOML spec is the single authoring surface for ``pyllm.tdg``: the API
surface, coverable rules and edge cases, optional ``sequenceDiagram`` blocks
(parsed by the existing :mod:`pyllm.tdg.sequence`), and — in *plan mode* —
...
- **plan mode** — the spec carries ``[[cases]]``; :meth:`TomlSpec.to_test_plan`
  builds a :class:`~pyllm.tdg.schema.TestPlan` deterministically, with no LLM
