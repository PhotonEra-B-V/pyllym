...
class PlanSafetyError(ValueError):
    ...
    (see :mod:`~pyllm.tdg.safety`): a human must still review generated tests
...
async def build(
    ...
    **chat_kwargs: Any,
    ...
    rules/edge_cases is 'request' mode (:func:`~pyllm.tdg.planner.
    ...
    tables (parsed by :mod:`~pyllm.tdg.sequence`). The plan is then cross-checked
    deterministically (:func:`~pyllm.tdg.checks.validate_plan`): every scenario,
    ...
    for its real dependencies (:func:`~pyllm.tdg.deps.inspect_dependencies` —
    ...
    an AST safety gate (:func:`~pyllm.tdg.safety.scan_plan`): a case's
