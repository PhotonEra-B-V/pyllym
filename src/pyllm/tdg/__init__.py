...
:mod:`~pyllm.tdg.sequence`), and — in *plan mode* — hand-written ``[[cases]]``.
...
Or from the shell::
    ...
    python -m pyllm.tdg features/ --out tests/generated --model gpt-5.4

The LLM only ever fills the :class:`~pyllm.tdg.schema.TestPlan` schema; test
