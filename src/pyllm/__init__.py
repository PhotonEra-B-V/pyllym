"""pyllm — one delightful, async-first Python framework for every major AI
provider.
...
def configure(block: Callable[[Configuration], Any]) -> Configuration:
    """Configure pyllm: ``pyllm.configure(lambda c: setattr(c, 'openai_api_key', ...))``."""
    cfg = config()
    block(cfg)
    return cfg
...
def list_providers() -> list[type[Provider]]:
    ...
    Note: ``pyllm.providers`` is the provider *subpackage*; use this function
...
class _ModelsProxy:
    """Attribute proxy so ``pyllm.models.find(...)`` reaches the registry module
    even though the ``models`` attribute on the package shadows the submodule."""

    def __getattr__(self, name: str) -> Any:
        import importlib

        module = importlib.import_module("pyllm.models")
