"""Celery task factory bridging pyllm's async API into synchronous workers."""
...
def run_async(coro: Coroutine[Any, Any, T]) -> T:
    """Run a pyllm coroutine to completion on a fresh event loop.

    pyllm's shared HTTP pools are keyed per event loop, so they are closed
...
def create_tasks(app: Celery, *, name_prefix: str = "pyllm", **task_options: Any) -> Tasks:
    """Register pyllm tasks on ``app`` and return them as a :class:`Tasks`.
    ...
    def ask(
        ...
        *,
        ...
        chat = pyllm.create_chat(model=model, provider=provider)
    ...
    def embed(
        ...
        *,
        ...
        result = run_async(pyllm.embed(text, model=model, provider=provider, dimensions=dimensions))
        return {
    ...
    def speak(
        ...
        *,
        ...
        speech = run_async(
            pyllm.speak(input, model=model, provider=provider, voice=voice, format=format)
        )
        if save_path is not None:
    ...
    def transcribe(
        ...
        *,
        ...
        result = run_async(
            pyllm.transcribe(audio_file, model=model, provider=provider, language=language)
        )
        data = {
            "text": result.text,
            "model": result.model,
    ...
    def moderate(
        ...
        *,
        ...
        result = run_async(pyllm.moderate(input, model=model, provider=provider))
        return {
            "id": result.id,
            "model": result.model,
