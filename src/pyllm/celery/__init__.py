"""Optional Celery integration for running pyllm operations on workers.

Requires the ``celery`` extra (``pip install pyllm[celery]``).
...
a fresh event loop via :func:`run_async`, which also closes pyllm's shared
HTTP pools before the loop shuts down. Use :func:`run_async` directly when
writing your own tasks around richer pyllm features (tools, agents,
...
try:
    ...
    raise ModuleNotFoundError(
        "pyllm.celery requires the 'celery' extra: pip install 'pyllm[celery]'"
    ) from exc
