...
:mod:`~pyllym.bdd.sequence`), and — in *plan mode* — hand-written ``[[cases]]``.
...
Or from the shell::
    ...
    python -m pyllym.bdd features/ --out tests/generated --model gpt-5.4

The LLM only ever fills the :class:`~pyllym.bdd.schema.TestPlan` schema; test
