...
A TOML spec is the single authoring surface for ``pyllym.bdd``: the API
surface, coverable rules and edge cases, optional ``sequenceDiagram`` blocks
(parsed by the existing :mod:`pyllym.bdd.sequence`), and — in *plan mode* —
...
- **plan mode** — the spec carries ``[[cases]]``; :meth:`TomlSpec.to_test_plan`
  builds a :class:`~pyllym.bdd.schema.TestPlan` deterministically, with no LLM
