...
class PlanSafetyError(ValueError):
    ...
    (see :mod:`~pyllym.bdd.safety`): a human must still review generated tests
...
async def build(
    ...
    **chat_kwargs: Any,
    ...
    rules/edge_cases is 'request' mode (:func:`~pyllym.bdd.planner.
    ...
    tables (parsed by :mod:`~pyllym.bdd.sequence`). The plan is then cross-checked
    deterministically (:func:`~pyllym.bdd.checks.validate_plan`): every scenario,
    ...
    an AST safety gate (:func:`~pyllym.bdd.safety.scan_plan`): a case's
