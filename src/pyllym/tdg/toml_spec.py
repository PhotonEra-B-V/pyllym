"""TOML front-end for the TDG builder — one spec file, two modes.

A TOML spec is the single authoring surface for ``pyllym.tdg``: the API
surface, coverable rules and edge cases, optional ``sequenceDiagram`` blocks
(parsed by the existing :mod:`pyllym.tdg.sequence`), and — in *plan mode* —
...
- **plan mode** — the spec carries ``[[cases]]``; :meth:`TomlSpec.to_test_plan`
  builds a :class:`~pyllym.tdg.schema.TestPlan` deterministically, with no LLM
