...
Requests are SigV4-signed by :class:`pyllym.providers.bedrock.Bedrock` via its
...
logger = __import__("logging").getLogger("pyllym")
...
class Converse(Protocol):
    ...
    def _warn_unsupported_citations(self, model: Info) -> None:
        logger.warning(
            "pyllym does not support citations on Bedrock yet. Ignoring with_citations for %s.",
            model.id,
        )
