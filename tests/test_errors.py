...
import pyllym
from pyllym.errors import BadRequestError, RateLimitError, ServerError, UnauthorizedError
...
async def test_429_retries_then_raises_rate_limit(mock_http):
    pyllym.configure(lambda c: setattr(c, "max_retries", 0))  # no backoff waits
    mock_http.post(URL, status=429, payload={"error": {"message": "slow down"}})
    with pytest.raises(RateLimitError):
        await pyllym.create_chat(model="gpt-4o").ask("hi")
    assert sent_requests(mock_http)
    pyllym.configure(lambda c: setattr(c, "max_retries", 3))


@pytest.mark.asyncio
async def test_500_maps_to_server_error(mock_http):
    pyllym.configure(lambda c: setattr(c, "max_retries", 0))
