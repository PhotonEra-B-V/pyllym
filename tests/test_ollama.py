...
import pyllym
from pyllym.providers.ollama import Ollama
...
@pytest.fixture
def _ollama_config():
    """Point pyllym at a local Ollama endpoint for the duration of a test."""
    cfg = pyllym.config()
...
async def test_ollama_chat(_ollama_config):
    with aioresponses() as m:
        ...
        chat = pyllym.create_chat(model="llama3", provider="ollama", assume_model_exists=True)
...
async def test_ollama_bearer_header_with_key(_ollama_config):
    ...
    pyllym.config().ollama_api_key = "secret-token"
    with aioresponses() as m:
        m.post(f"{OLLAMA_BASE}/chat/completions", payload=f.openai_chat("ok"))
        chat = pyllym.create_chat(model="llama3", provider="ollama", assume_model_exists=True)
        await chat.ask("hi")
