...
async def test_embedding_single_vector(mock_http):
    ...
    emb = await pyllym.embed("hello", model="text-embedding-3-small")
...
async def test_embedding_batch_vectors(mock_http):
    ...
    emb = await pyllym.embed(["a", "b"], model="text-embedding-3-small")
...
async def test_moderation_flagged(mock_http):
    ...
    result = await pyllym.moderate("something", model="omni-moderation-latest")
...
async def test_image_generation(mock_http):
    ...
    image = await pyllym.paint("a cat", model="gpt-image-1.5")
...
async def test_chat_from_seeded_fixture_file(mock_http):
    ...
    msg = await pyllym.create_chat(model="gpt-4o").ask("hi")
